...
CREATE OR REPLACE SECURE VIEW DAILY_SALES
AS
SELECT DISTINCT
...
GROUP BY DIM_STORE.STORENUMBER, DIM_DATE.DATE, DIM_DATE.YEAR
ORDER BY DIM_STORE.STORENUMBER, DIM_DATE.DATE;

CREATE OR REPLACE SECURE VIEW DAILY_TARGET
AS
SELECT DISTINCT
    DIM_STORE.STORENUMBER,
    DIM_DATE.DATE,
...
CREATE OR REPLACE SECURE VIEW VW_DAILY_SALES_TARGET
AS
SELECT DISTINCT DAILY_SALES.STORENUMBER,
    ...
    END AS TARGETMET
FROM DAILY_SALES
LEFT OUTER JOIN DAILY_TARGET ON DAILY_SALES.DATE = DAILY_TARGET.DATE
WHERE DAILY_SALES.STORENUMBER = DAILY_TARGET.STORENUMBER
ORDER BY DAILY_SALES.STORENUMBER, DAILY_SALES.DATE;
